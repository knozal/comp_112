...
garden_harvest %>% 
  ...
  summarize(total_weight_lbs = sum(weight*0.00220462)) %>% 
  ggplot() +
    geom_col(aes(x=total_weight_lbs, y= variety))
...
mutate(exact_time = ) %>% 
ggplot() +
  geom_density(aes(exact_time, fill = `client`, alpha = 0.5, color=NA))
```
...
mutate(exact_time = ) %>% 
ggplot() +
  geom_density(aes(exact_time, fill = `client`, alpha = 0.5, color=NA, position = position_stack()))
```
